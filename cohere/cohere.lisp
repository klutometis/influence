   ...
   ; 2. COH-POINT-5 is an incremental approximation algorithm that guarantees
   that at
   ...
   ; 4. CONNECT-SOLUTION uses a connectionist algorithm to accept and reject
   elements.
   ; This function assumes that a network of constraints has already been set
   up by
   ...
   (defun EVAL-COHERE ()
   (my-print "=====================")
   (my-print "Evaluating coherence for problem " *problem* " in mode "
   *eval-mode*)
   ...
   rejected-list)
   ; where number is the number of the solution, %1 is the decimal fraction
   of
   ...
   ; repeat:
   (unless (and (equal mode 'foundational) ; if foundational, ignore
   solutions that
   (not (subsetp favored (car element-lists))) ; don't accept favored
   elements
   )
   ...
   ; GET-FAVORED returns a list of all favored elements, i.e. ones with links
   to SPECIAL
   ...
   ; COH-SCORE-ALL revises a list of solutions of the form (number accepted
   rejected)
   ...
   ; LIST-CONSTRAINTS generates a list of all constraints using the links
   associated
   ...
   (list (roundoff 3 (float (/ weights-satisfied
   *weight-of-all-constraints*))) ; %weight
   ...
   ; CONS-SATISFIED checks to see if a constraint is satisfied, where a
   constraint
   ; has the forum (unit1 unit2 weight). If the weight is positive, then the
   units
   ; should be either both accepted or both rejected. If the weight is
   negative,
   ...
   (terpri) (my-print "No more than " *show-solutions* " solutions are
   shown.")
   )
   (print (car solutions))
   )
   ...
   ; BEST-SOLUTION prints out the best solution, or the best solutions if
   there are ties.
   ...
   (sort (mapcar #'resonate result) #'> :key #'last-el) ; return ordered by
   resonance
   )
   ; repeat
   ...
   (do ((units (randomize *all-units*) (cdr units)) ; put units in random
   order
   (accepted '(special))
   (rejected nil)
   (unit nil)
   ...
   (cons 'incremental (append (coh-score (list 'incremental accepted
   rejected))
   ...
   ; ASSOC-NUMS produces an association list of (number unit). This is like
   make-units
   ; above.
   ...
   ; COH* calculates the coherence of a set of units partitioned into
   accepted and
   ; rejected.
